...
struct ParakeetEngineTests {
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("superduper-parakeet-unit-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func appModelDirectoryMatchesFluidAudioRepoLayout() {
        let base = URL(fileURLWithPath: "/tmp/superduper-base", isDirectory: true)
    ...
    @Test func resolverReturnsNilWithoutCompleteModels() {
        let resolved = ParakeetEngine.resolveModelDirectory(
            version: .v2,
            downloadBase: URL(fileURLWithPath: "/tmp/superduper-base", isDirectory: true),
