...
struct SettingsPresentationTests {
    ...
    @Test func presetsForPickerIncludesSixCatalogPresets() {
        ...
        #expect(list.count == SuperduperThemePresetCatalog.presets.count)
        #expect(list.map(\.id) == SuperduperThemePresetCatalog.presets.map(\.id))
    ...
    @Test func presetsForPickerAppendsLegacyGraphiteOnlyWhenSelected() {
        ...
        #expect(withLegacy.count == SuperduperThemePresetCatalog.presets.count + 1)
    ...
    @Test func logExportListsRegularFilesOnly() throws {
        let temp = FileManager.default.temporaryDirectory
            .appendingPathComponent("superduper-log-export-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func orbWaveformPaletteDerivesUnspecifiedPresetFromAccent() {
        ...
        let catalogAccent = SuperduperThemePresetCatalog
    ...
    @Test func orbWaveformPaletteTracksVariantAccentForDerivedPresets() {
        for presetID in ["paper", "evergreen", "signal"] {
            for variant in [SuperduperThemeVariant.light, .dark] {
