...
struct ThemeFoundationTests {
    ...
    @Test func libraryIsDefaultPreset() {
        #expect(SuperduperThemePresetCatalog.defaultPresetID == "library")
        let fallback = SuperduperThemePresetCatalog.preset(withID: nil)
        #expect(fallback.id == "library")
        let unknown = SuperduperThemePresetCatalog.preset(withID: "does-not-exist")
        #expect(unknown.id == "library")
    }

    @Test func graphiteIsLegacyResolvableButHiddenFromPicker() {
        #expect(SuperduperThemePresetCatalog.presets.contains(where: { $0.id == "graphite" }) == false)
        #expect(SuperduperThemePresetCatalog.legacyPresets.contains(where: { $0.id == "graphite" }) == true)
        let graphite = SuperduperThemePresetCatalog.preset(withID: "graphite")
        #expect(graphite.id == "graphite")
        #expect(graphite.isLegacy == true)
    }

    @Test func pickerIncludesScorchedPresets() {
        let ids = Set(SuperduperThemePresetCatalog.presets.map(\.id))
        #expect(ids == Set(["library", "superduper", "paper", "harbor", "evergreen", "signal"]))
    }

    @Test func renamedPresetIDsStillResolve() {
        #expect(SuperduperThemePresetCatalog.preset(withID: "pindrop").id == "superduper")
    }

    @Test func signaturePresetAccentHexesArePinned() {
        // Signature accents from the redesign brief (personality carriers).
        #expect(SuperduperThemePresetCatalog.superduper.darkTheme.accentHex.uppercased() == "#F2B54A")
        #expect(SuperduperThemePresetCatalog.paper.lightTheme.accentHex.uppercased() == "#2E4E73")
        #expect(SuperduperThemePresetCatalog.harbor.lightTheme.accentHex.uppercased() == "#14708A")
        #expect(SuperduperThemePresetCatalog.evergreen.lightTheme.accentHex.uppercased() == "#4D7A4A")
        #expect(SuperduperThemePresetCatalog.signal.darkTheme.accentHex.uppercased() == "#F06D4F")
    }
    ...
    @Test func allPresetsProduceValidClampedPalettesLightAndDark() {
        for preset in SuperduperThemePresetCatalog.allPresets {
            for variant in SuperduperThemeVariant.allCases {
    ...
    @Test func libraryLightTokensMatchSpec() {
        let profile = SuperduperThemePresetCatalog.library.lightTheme
    ...
    @Test func libraryDarkTokensMatchSpec() {
        let profile = SuperduperThemePresetCatalog.library.darkTheme
    ...
    @Test func reducedTransparencyUsesOpaqueInkOverlaySurface() throws {
        let palette = ResolvedPalette.resolve(
            profile: SuperduperThemePresetCatalog.library.lightTheme,
            isDark: false,
            displayOptions: AccessibilityDisplayOptions(reduceTransparency: true)
        ).palette
        let expected = try #require(NSColor(superduperHex: "#181511"))
