...
//  coverage lives in WhisperKitEngineIntegrationTests (SUPERDUPER_RUN_INTEGRATION_TESTS).
//
...
struct WhisperKitEngineTests {
    ...
    private func makeEmptyDownloadBase() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("superduper-whisperkit-unit-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        return url
    }

    private func nonexistentModelPath() -> String {
        "/nonexistent/superduper/whisperkit/\(UUID().uuidString)/MelSpectrogram.mlmodelc"
    }
...
/// Run with `SUPERDUPER_RUN_INTEGRATION_TESTS=1 just test-integration` (or equivalent).
@MainActor
@Suite(
    "WhisperKitEngine (integration, network)",
    .enabled(
        if: ProcessInfo.processInfo.environment["SUPERDUPER_RUN_INTEGRATION_TESTS"] == "1",
        "WhisperKit network/model-download tests are disabled by default. Run with SUPERDUPER_RUN_INTEGRATION_TESTS=1."
    )
)
struct WhisperKitEngineIntegrationTests {
    @Test func loadTinyModelByNameDownloadsAndSetsReady() async throws {
        ...
        // Intentionally allows network — gated by SUPERDUPER_RUN_INTEGRATION_TESTS.
