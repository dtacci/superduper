...
struct MentionRewriteServiceTests {
    private func makeSUT() -> (
        ...
        mockFS.filesByRoot = [
            "/workspace": [
                "/workspace/SuperduperDictation/Services/AppCoordinator.swift",
                "/workspace/SuperduperDictation/Services/AudioRecorder.swift",
                "/workspace/SuperduperDictation/Services/TranscriptionService.swift",
                "/workspace/SuperduperDictation/UI/Settings/SettingsWindow.swift",
                "/workspace/SuperduperDictation/UI/Main/MainWindow.swift",
                "/workspace/SuperduperDictation/Utils/Logger.swift",
                "/workspace/SuperduperDictation/Models/TranscriptionRecord.swift",
                "/workspace/SuperduperDictationTests/AudioRecorderTests.swift",
                "/workspace/README.md",
            ]
        ]
    ...
    @Test func deriveWorkspaceInsightsIncludesActiveDocumentAndTags() async {
        ...
        let insights = await fixture.sut.deriveWorkspaceInsights(
            workspaceRoots: ["/workspace"],
            activeDocumentPath: "/workspace/SuperduperDictation/Services/AppCoordinator.swift"
        )

        #expect(insights.activeDocumentRelativePath == "SuperduperDictation/Services/AppCoordinator.swift")
        ...
        #expect(insights.fileTagCandidates.first == "SuperduperDictation/Services/AppCoordinator.swift")
    }
    ...
    @Test func vsCodeFormatsExactMentionPath() async {
        ...
        #expect(result.didRewrite)
        #expect(result.text.contains("@SuperduperDictation/Services/AppCoordinator.swift"))
    }
    ...
    @Test func normalizeFilePathToParentDirectory() {
        ...
        mockFS.directories.insert("/workspace/SuperduperDictation/Services")
        ...
        #expect(result == ["/workspace/SuperduperDictation/Services"])
    }

    @Test func normalizeTildePath() {
        let fixture = makeSUT()
        let home = NSHomeDirectory()
        let expandedPath = "\(home)/Projects/superduper"
        var mockFS = fixture.mockFS
        mockFS.directories.insert(expandedPath)
        let sut = MentionRewriteService(fileSystem: mockFS)

        let result = sut.normalizeWorkspaceRoots(["~/Projects/superduper"])
    ...
    @Test func normalizeClimbsToProjectMarker() {
        ...
        var mockFS = fixture.mockFS
        mockFS.directories.insert("/workspace/SuperduperDictation/Services")
        mockFS.directories.insert("/workspace/SuperduperDictation")
    ...
    @Test func normalizeWithFileURLAndTildeAndFilePath() {
        let fixture = makeSUT()
        let home = NSHomeDirectory()
        let projectRoot = "\(home)/Projects/superduper"
        var mockFS = fixture.mockFS
        mockFS.directories.insert(projectRoot)
        mockFS.directories.insert("\(projectRoot)/SuperduperDictation")
        mockFS.directories.insert("\(projectRoot)/.git")
        let sut = MentionRewriteService(fileSystem: mockFS)

        let result = sut.normalizeWorkspaceRoots([
            "file://\(projectRoot)/SuperduperDictation/AppCoordinator.swift"
        ])
    ...
    @Test func rewriteWorksWithFilePathAsWorkspaceRoot() async {
        ...
        let result = await fileSut.rewrite(
            text: "check AppCoordinator.swift",
            capabilities: fixture.cursorCapabilities,
            workspaceRoots: ["/workspace/SuperduperDictation/Services/AppCoordinator.swift"]
        )
    ...
    @Test func antigravityAdapterRewriteWithTildeRedactedRoot() async {
        ...
        mockFS.directories.insert(projectRoot)
        mockFS.directories.insert("\(projectRoot)/SuperduperDictation")
        mockFS.directories.insert("\(projectRoot)/SuperduperDictation/Services")
        mockFS.directories.insert("\(projectRoot)/.git")
        mockFS.filesByRoot[projectRoot] = [
            "\(projectRoot)/SuperduperDictation/Services/AppCoordinator.swift",
            "\(projectRoot)/SuperduperDictation/Services/AudioRecorder.swift",
        ...
        let result = await sut.rewrite(
            ...
            workspaceRoots: ["~/Projects/superduper"]
        )
    ...
    @Test func normalizeTildeRedactedFilePath() {
        ...
        let result = sut.normalizeWorkspaceRoots([
            "~/Projects/superduper/SuperduperDictation/AppCoordinator.swift"
        ])
    ...
    @Test func routingSignalDerivesWorkspaceRootFromDocumentPath() {
        let snapshot = ContextSnapshot(
            ...
            appContext: AppContextInfo(
                ...
                selectedText: nil,
                documentPath: "~/Projects/superduper/SuperduperDictation/AppCoordinator.swift",
                browserURL: nil
            ),
            clipboardText: nil,
            warnings: []
        ...
        #expect(signal.workspacePath == "~/Projects/superduper/SuperduperDictation")
    }
    ...
    @Test func routingSignalDerivesWorkspaceRootFromFileURL() {
        let snapshot = ContextSnapshot(
            ...
            appContext: AppContextInfo(
                ...
                focusedElementValue: nil,
                selectedText: nil,
                documentPath: "file:///Users/test/Projects/superduper/file.swift",
                browserURL: nil
            ),
            clipboardText: nil,
            warnings: []
        ...
        #expect(signal.workspacePath == "/Users/test/Projects/superduper")
    }
    ...
    @Test func routingSignalUsesDirectoryDocumentPathForTerminalApps() {
        let snapshot = ContextSnapshot(
            ...
            appContext: AppContextInfo(
                ...
                focusedElementValue: nil,
                selectedText: nil,
                documentPath: "~/Projects/superduper/",
                browserURL: nil
            ),
            clipboardText: nil,
            warnings: []
        ...
        #expect(signal.workspacePath == "~/Projects/superduper")
    }
    ...
    private func makeEditorIntegrationSut() -> MentionRewriteService {
        ...
        let home = NSHomeDirectory()
        let root = "\(home)/Projects/superduper"
        fs.directories = [
            root,
            "\(root)/.git",
            "\(root)/SuperduperDictation",
            "\(root)/SuperduperDictation/Services",
        ]
        fs.filesByRoot = [
            root: [
                "\(root)/SuperduperDictation/Services/AppCoordinator.swift",
                "\(root)/SuperduperDictation/Services/AudioRecorder.swift",
                "\(root)/README.md",
            ]
        ]
    ...
    @Test func cursorDerivationAndRewriteFromFilePath() async {
        ...
        let result = await sut.rewrite(
            text: "open AppCoordinator.swift",
            capabilities: CursorAdapter().capabilities,
            workspaceRoots: ["\(home)/Projects/superduper/SuperduperDictation/Services/AppCoordinator.swift"]
        )
    ...
    @Test func vsCodeDerivationAndRewriteFromFileURL() async {
        ...
        let result = await sut.rewrite(
            text: "open AppCoordinator.swift",
            capabilities: VSCodeAdapter().capabilities,
            workspaceRoots: ["file://\(home)/Projects/superduper/SuperduperDictation/Services/AppCoordinator.swift"]
        )
    ...
    @Test func windsurfDerivationAndRewriteFromSubdirectory() async {
        ...
        let result = await sut.rewrite(
            text: "open AppCoordinator.swift",
            capabilities: WindsurfAdapter().capabilities,
            workspaceRoots: ["\(home)/Projects/superduper/SuperduperDictation/Services"]
        )
    ...
    @Test func allEditorsPrefixDeterminism() async {
        ...
        for (name, caps, expectedPrefix) in editors {
            ...
            #expect(
                result.text.contains("\(expectedPrefix)SuperduperDictation/Services/AppCoordinator.swift"),
