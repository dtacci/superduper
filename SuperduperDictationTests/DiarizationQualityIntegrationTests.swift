...
private let diarizationIntegrationPrerequisitesAvailable: Bool = {
    guard ProcessInfo.processInfo.environment["SUPERDUPER_RUN_INTEGRATION_TESTS"] == "1" else { return false }
    let configuredRoot = ProcessInfo.processInfo.environment["SUPERDUPER_DIARIZATION_FIXTURE_ROOT"]
    let root = configuredRoot.map { URL(fileURLWithPath: $0, isDirectory: true) }
        ?? URL(fileURLWithPath: "SuperduperDictationTests/Fixtures/Diarization/Generated", isDirectory: true)
...
struct DiarizationQualityIntegrationTests {
    ...
    private static var fixtureRoot: URL {
        if let configured = ProcessInfo.processInfo.environment["SUPERDUPER_DIARIZATION_FIXTURE_ROOT"] {
            return URL(fileURLWithPath: configured, isDirectory: true)
        }
        return URL(fileURLWithPath: "SuperduperDictationTests/Fixtures/Diarization/Generated", isDirectory: true)
    }
