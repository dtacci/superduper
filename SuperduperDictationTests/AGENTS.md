...
├── SuperduperDictationTests.swift              # Base template
...
```bash
xcodebuild test -scheme SuperduperDictation -testPlan Unit -destination 'platform=macOS'
xcodebuild test -scheme SuperduperDictation -testPlan Integration -destination 'platform=macOS'
just test
