...
struct SenseVoiceEngineTests {
    ...
    @Test func loadModelWithMissingPathFails() async {
        let engine = makeEngine()
        do {
            try await engine.loadModel(path: "/tmp/superduper-sensevoice-missing-\(UUID().uuidString)")
    ...
    @Test func loadModelPathWithFp16OnlyCacheFailsForCatalogInt8() async throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("superduper-sensevoice-fp16-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func discoveryDecisionMatchesCatalogInt8Precision() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("superduper-sensevoice-discovery-\(UUID().uuidString)", isDirectory: true)
