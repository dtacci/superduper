...
struct SettingsStoreAIConfigTests {
   ...
   /// the migrator against that state. This mirrors the upgrade path from a prior Superduper Dictation
   /// version.
   private func migrateAfterSeeding(
      ...
      // Superduper Dictation's SettingsStore reads from a per-PID UserDefaults suite during tests, so a
