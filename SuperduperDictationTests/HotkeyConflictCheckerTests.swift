...
struct HotkeyConflictCheckerTests {
    ...
    func crossSlotConflictDetected() {
        ...
        #expect(status == .superduperConflict(conflictingSlot: .toggleRecording))
    }
    ...
    @Test("Cancel Operation conflicts with an existing SuperduperDictation slot")
    ...
    @Test("SuperduperDictation conflict takes precedence over system shortcut")
    func superduperPrecedenceOverSystem() {
        ...
        #expect(status == .superduperConflict(conflictingSlot: .pushToTalk))
    }
