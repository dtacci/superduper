...
final class NoteEditorWindowController: NSObject, NSWindowDelegate {
    ...
    private func presentEditor(note: NoteSchema.Note?, isNewNote: Bool) {
        ...
        self.window = window
        registerAsLive()
        themeCancellable = SuperduperThemeController.shared.$revision.sink { [weak self] _ in
            guard let self else { return }
            SuperduperThemeController.shared.apply(to: self.window)
        }
...
actor NoteEditorPersistenceActor {
    ...
    func save(
        ...
        if alreadyCurrent {
            ...
            try modelContext.save()
            if tagsChanged {
                NotificationCenter.default.post(name: .superduperNoteTagsDidChange, object: nil)
            }
        }
