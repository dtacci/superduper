...
final class PillFloatingIndicatorController: NSObject, ObservableObject, NSMenuDelegate, FloatingIndicatorPresenting,
    ...
    private func checkAndUpdateScreenPosition(force: Bool = false) {
        ...
        if !force, let last = lastScreen, currentScreen.superduper_isSameDisplay(as: last) {
            return
        }
    ...
    func panelDidReceiveLeftMouseEvent(_ event: NSEvent) {
        switch event.type {
        case .leftMouseDown:
            pendingDragStartScreenPoint = event.superduper_screenLocation
            ...
            HStack(spacing: showsTranscript ? 8 : 9) {
                ...
                if showsTranscript {
                    Text(FloatingIndicatorTimeFormatting.elapsed(state.recordingDuration))
                        ...
                        .foregroundStyle(Color(nsColor: NSColor(superduperHex: "#A59D8C") ?? .secondaryLabelColor))
