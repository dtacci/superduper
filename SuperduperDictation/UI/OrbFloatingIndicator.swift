...
final class OrbFloatingIndicatorController: NSObject, ObservableObject, FloatingIndicatorPresenting,
    ...
    func panelDidReceiveLeftMouseEvent(_ event: NSEvent) {
        switch event.type {
        case .leftMouseDown:
            let screenPoint = event.superduper_screenLocation
        ...
        case .leftMouseUp:
            ...
            if isDragging {
                ...
                handlePanelTap(atScreenPoint: event.superduper_screenLocation)
            }
        default:
            break
        }
    ...
    func panelDidReceiveRightMouseDown(_ event: NSEvent) -> Bool {
        guard allowsHitTesting(atScreenPoint: event.superduper_screenLocation) else {
            return false
        }
    ...
    private func checkScreenPosition(force: Bool = false) {
        ...
        if force || lastScreen?.superduper_isSameDisplay(as: currentScreen) == false {
...
struct OrbIndicatorView: View {
    ...
    private var waveformPalette: OrbWaveformPalette {
        ...
        let key = isDark
            ? SuperduperThemeStorageKeys.darkThemePresetID
            : SuperduperThemeStorageKeys.lightThemePresetID
    ...
    private var pillTopRow: some View {
        if state.isRecording {
            HStack(spacing: 9) {
                Circle()
                    .fill(Color(nsColor: NSColor(superduperHex: "#D25B4C") ?? .systemRed))
                ...
                Text(formattedDuration)
                    ...
                    .foregroundStyle(
                        Color(nsColor: NSColor(superduperHex: showsTranscript ? "#A59D8C" : "#EFEBE2") ?? .white)
                    )
                ...
                Button { controller.handlePillStopTapped() } label: {
                    RoundedRectangle(cornerRadius: showsTranscript ? 2 : 2.5, style: .continuous)
                        .fill(Color(nsColor: NSColor(superduperHex: "#EFEBE2") ?? .white).opacity(0.72))
                        .frame(
            ...
            HStack(spacing: 9) {
                ...
                Text(processingStatusText)
                    ...
                    .foregroundStyle(Color(nsColor: NSColor(superduperHex: "#EFEBE2") ?? .white))
                    .lineLimit(1)
                Spacer(minLength: 0)
            }
...
/// The superduper.dev orb, ported to the indicator: an amorphous blob whose
...
private struct OrbBlobFillView: View {
    ...
    private static func shifted(_ hex: String, hue hueDelta: CGFloat, sat satScale: CGFloat, bri briScale: CGFloat) -> Color {
        guard let ns = (NSColor(superduperHex: hex) ?? .controlAccentColor).usingColorSpace(.deviceRGB) else {
...
struct OrbWaveformPalette: Equatable {
    ...
    private struct CacheKey: Hashable {
        let presetID: String
        let variant: SuperduperThemeVariant
    ...
    static func cached(
        presetID: String?,
        variant: SuperduperThemeVariant,
        themeRevision: Int
    ) -> OrbWaveformPalette {
        let resolvedPresetID = presetID ?? SuperduperThemePresetCatalog.defaultPresetID
        let key = CacheKey(
    ...
    static func forPresetID(
        _ presetID: String?,
        variant: SuperduperThemeVariant = .light
    ) -> OrbWaveformPalette {
        switch presetID ?? SuperduperThemePresetCatalog.defaultPresetID {
        case "library":
        ...
        default:
            ...
            let accent = SuperduperThemePresetCatalog
    ...
    private static func color(_ hex: String) -> Color {
        Color(nsColor: NSColor(superduperHex: hex) ?? .controlAccentColor)
    }
}
...
enum OrbPalette {
    static let surface = Color(nsColor: NSColor(superduperHex: "#181511") ?? .black).opacity(0.92)
