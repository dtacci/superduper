...
private struct ToastView: View {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: ToastMetrics.cornerRadius, style: .continuous)
                ...
                .fill(Color(nsColor: NSColor(superduperHex: "#201D18") ?? .black))
        )
    ...
    private var leadingColor: Color {
        ...
        case .microphoneUnavailable:
            return Color(nsColor: NSColor(superduperHex: "#D25B4C") ?? .systemRed)
        case .standard where payload.style == .error:
            return Color(nsColor: NSColor(superduperHex: "#D25B4C") ?? .systemRed)
        default:
            return Color(nsColor: NSColor(superduperHex: "#4CA582") ?? .systemGreen)
        }
    }
...
private struct ToastActionButtonStyle: ButtonStyle {
    ...
    private func foregroundColor(isPressed: Bool) -> Color {
        ...
        case .secondary:
            return Color(nsColor: NSColor(superduperHex: "#A59D8C") ?? .secondaryLabelColor)
                .opacity(isPressed ? 0.7 : 1)
        }
    }
}
