...
extension NSScreen {
    ...
    var superduper_displayNumber: UInt32 {
        (deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? NSNumber)?.uint32Value ?? 0
    }

    /// Whether two screens refer to the same physical display.
    func superduper_isSameDisplay(as other: NSScreen?) -> Bool {
        guard let other else { return false }
        let a = superduper_displayNumber
        let b = other.superduper_displayNumber
        if a != 0, b != 0 {
            return a == b
        }
        return self === other
...
extension Timer {
    ...
    @MainActor
    static func superduper_scheduleRepeating(interval: TimeInterval, block: @escaping (Timer) -> Void) -> Timer {
...
extension NSEvent {
    ...
    var superduper_screenLocation: NSPoint {
...
private struct _IndicatorProcessingDot: View {
    ...
    var body: some View {
        ...
        Circle()
            .fill(
                Color(nsColor: NSColor(superduperHex: "#4CA582") ?? .systemGreen)
...
struct FloatingIndicatorWaveformStyle {
    ...
    static let pill = FloatingIndicatorWaveformStyle(
        ...
        maximumHeight: 12,
        idleHeight: 4,
        color: Color(nsColor: NSColor(superduperHex: "#4CA582") ?? .systemGreen),
...
final class WaveformFloatingIndicatorController: NSObject, FloatingIndicatorPresenting,
    ...
    func panelDidReceiveLeftMouseEvent(_ event: NSEvent) {
        switch event.type {
        case .leftMouseDown:
            dragOrigin = event.superduper_screenLocation
            dragStartOffset = settingsStore.waveformFloatingIndicatorOffset
            isDragging = false
        case .leftMouseDragged:
            guard let dragOrigin else { return }
            let point = event.superduper_screenLocation
...
final class FloatingIndicatorState: ObservableObject {
    ...
    private func startDurationTimer() {
        ...
        durationTimer = Timer.superduper_scheduleRepeating(interval: 0.1) { [weak self] _ in
            Task { @MainActor in
