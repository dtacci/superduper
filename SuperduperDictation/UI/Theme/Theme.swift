...
@MainActor
final class SuperduperThemeController: ObservableObject {
    static let shared = SuperduperThemeController()
    ...
    private var currentMode: SuperduperThemeMode {
        let rawValue = UserDefaults.standard.string(forKey: SuperduperThemeStorageKeys.themeMode)
        return SuperduperThemeMode(rawValue: rawValue ?? "") ?? .system
    }
}

private struct ThemeRefreshModifier: ViewModifier {
    @ObservedObject private var theme = SuperduperThemeController.shared
...
enum AppColors {
    ...
    private static func resolvedPalette(for isDark: Bool) -> ResolvedPalette {
        let variant: SuperduperThemeVariant = isDark ? .dark : .light
        let storageKey = isDark ? SuperduperThemeStorageKeys.darkThemePresetID : SuperduperThemeStorageKeys.lightThemePresetID
        ...
        let profile = SuperduperThemePresetCatalog.profile(for: presetID, variant: variant)
...
struct ResolvedPalette {
    ...
    init(
        profile: SuperduperThemeProfile,
        isDark: Bool,
    ...
    static func resolve(
        ...
        let ground = NSColor(superduperHex: profile.groundHex)
            ?? (isDark
                ? NSColor(superduperHex: "#1B1916")!
                : NSColor(superduperHex: "#F6F4EE")!)
        let page = NSColor(superduperHex: profile.pageHex)
            ?? (isDark
                ? NSColor(superduperHex: "#242119")!
                : NSColor(superduperHex: "#FCFBF7")!)
        let accentBase = NSColor(superduperHex: profile.accentHex) ?? .systemGreen
        ...
        let ink = NSColor(superduperHex: inkHex) ?? (isDark ? .white : .black)
        var ink2 = NSColor(superduperHex: ink2Hex) ?? ink.withAlphaComponent(0.72)
        var ink3 = NSColor(superduperHex: ink3Hex) ?? ink.withAlphaComponent(0.48)
        let baseLine = NSColor(superduperHex: lineHex) ?? ink.withAlphaComponent(0.12)
        ...
        if let softHex = profile.accentSoftHex, let soft = NSColor(superduperHex: softHex) {
            accentSoft = soft
        } else {
        ...
        let recordBase = NSColor(superduperHex: profile.recordHex ?? defaultRecordHex) ?? .systemRed
        let recordSoft: NSColor
        if let softHex = profile.recordSoftHex, let soft = NSColor(superduperHex: softHex) {
            recordSoft = soft
        } else if !isDark, let soft = NSColor(superduperHex: ScorchedEarthBaseTokens.lightRecordSoft) {
            recordSoft = soft
        } else {
        ...
        let warningBase = isDark
            ? (NSColor(superduperHex: "#D09B53") ?? .systemOrange)
            : (NSColor(superduperHex: "#A9692D") ?? .systemOrange)
    ...
    private static func overlaySurface(
        ...
        if reduceTransparency {
            return NSColor(superduperHex: strong ? "#100E0C" : "#181511") ?? .black
        }
        ...
        let base = NSColor(superduperHex: strong ? "#100E0C" : "#181511") ?? .black
...
extension NSColor {
    convenience init?(superduperHex: String) {
        let cleaned = superduperHex
