...
final class FloatingIndicatorController: FloatingIndicatorPresenting, FloatingIndicatorPanelInteractionHandling {
    ...
    func panelDidReceiveLeftMouseEvent(_ event: NSEvent) {
        switch event.type {
        case .leftMouseDown:
            isStopPressPending = stopControlRegion()?.contains(event.superduper_screenLocation) == true
        case .leftMouseUp:
            let wasPending = isStopPressPending
            isStopPressPending = false
            if wasPending,
               stopControlRegion()?.contains(event.superduper_screenLocation) == true {
    ...
    private func startScreenTracking() {
        ...
        screenTrackingTimer = Timer.superduper_scheduleRepeating(interval: 0.05) { [weak self] _ in
            Task { @MainActor in
                self?.checkAndUpdateScreenPosition()
            }
        }
    ...
    private func checkAndUpdateScreenPosition() {
        ...
        if lastScreen?.superduper_isSameDisplay(as: currentScreen) == false || lastScreen == nil {
