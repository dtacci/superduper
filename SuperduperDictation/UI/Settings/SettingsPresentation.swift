...
enum SettingsThemePresetPresentation {
    ...
    static func presetsForPicker(
        selectedID: String,
        catalog: [SuperduperThemePreset] = SuperduperThemePresetCatalog.presets,
        legacy: [SuperduperThemePreset] = SuperduperThemePresetCatalog.legacyPresets
    ) -> [SuperduperThemePreset] {
        var list = catalog
...
enum SettingsHotkeyConflictPresentation {
    ...
    static func aggregateStatus(
        ...
        if statuses.contains(where: {
            if case .superduperConflict = $0 { return true }
            return false
        }) {
