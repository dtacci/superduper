...
enum HotkeyConflictStatus: Equatable, Sendable {
    /// No Superduper Dictation or known system conflict.
    case noConflict
    /// Same combo is already used by another Superduper Dictation slot.
    case superduperConflict(conflictingSlot: HotkeySlot)
...
enum HotkeyConflictChecker {
    ...
    /// - Superduper Dictation-internal conflicts take precedence over system-shortcut soft warnings.
    ...
    static func check(
        ...
        for assignment in assignments {
            ...
            if assignment.keyCode == keyCode && assignmentModifiers == normalizedModifiers {
                return .superduperConflict(conflictingSlot: assignment.slot)
            }
        }
