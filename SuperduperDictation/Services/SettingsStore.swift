...
private enum SettingsStoreRuntime {
   ...
   static let appStorageStore: UserDefaults? = {
      ...
      let suiteName =
         ...
         ?? "com.dantacci.superduper-dictation.settings.tests.\(ProcessInfo.processInfo.processIdentifier)"
...
final class SettingsStore: ObservableObject {
   ...
   // MARK: - Default Values (Single Source of Truth)
    ...
    enum Defaults {
       ...
       static let selectedModel = "parakeet-tdt-0.6b-v2"
        ...
        static let themeMode = SuperduperThemeMode.system.rawValue
      static let lightThemePresetID = SuperduperThemePresetCatalog.defaultPresetID
      static let darkThemePresetID = SuperduperThemePresetCatalog.defaultPresetID
   ...
   var cancelRequiresDoubleEscape: Bool = Defaults.Hotkeys.cancelRequiresDoubleEscape
    ...
    @AppStorage(SuperduperThemeStorageKeys.themeMode, store: SettingsStoreRuntime.appStorageStore)
    var themeMode: String = Defaults.themeMode {
      didSet { notifyThemeDidChange() }
   }
   @AppStorage(SuperduperThemeStorageKeys.lightThemePresetID, store: SettingsStoreRuntime.appStorageStore)
   var lightThemePresetID: String = Defaults.lightThemePresetID {
      didSet { notifyThemeDidChange() }
   }
   @AppStorage(SuperduperThemeStorageKeys.darkThemePresetID, store: SettingsStoreRuntime.appStorageStore)
   ...
   var waveformFloatingIndicatorOffset: CGSize {
    ...
    var selectedThemeMode: SuperduperThemeMode {
       get { SuperduperThemeMode(rawValue: themeMode) ?? .system }
   ...
   var selectedLightThemePreset: SuperduperThemePreset {
      SuperduperThemePresetCatalog.preset(withID: lightThemePresetID)
   }

   var selectedDarkThemePreset: SuperduperThemePreset {
      SuperduperThemePresetCatalog.preset(withID: darkThemePresetID)
   }
   ...
   func inferredCustomLocalProvider(for endpoint: String?) -> CustomProviderType? {
    ...
    private func migrateRenamedThemePresetIDs() {
       let renamed = SuperduperThemePresetCatalog.renamedPresetIDs
